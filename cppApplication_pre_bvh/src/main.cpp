...
Eigen::Vector3d shoot_ray(const Ray &ray, int depth = 0) {
    ...
    if (nearest_hit.has_value()) {
        ...
        Eigen::Vector3d R = (ray.direction - 2.0 * ray.direction.dot(N) * N).normalized();
        Ray reflected_ray(hit_point + R * 1e-4, R);
        Eigen::Vector3d reflected_color = shoot_ray(reflected_ray, depth + 1);
